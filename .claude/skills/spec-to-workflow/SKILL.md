...
`profile` copies a pre-rolled native config into the agent worktree root
(`.claude/settings.local.json`, `.codex/config.toml`, or `.cursor/cli.json`, depending
on the adapter). `editor` allows Read/Glob/Edit/Write and denies Bash, Task, WebFetch,
and WebSearch. `reader` is read-only.
...
Be straight about two things here. First, a profile narrows the *project* config layer
only; a user-level `~/.claude/settings.json` merges on top of it, so on a permissive
machine the profile is not authoritative. Second, profiles are deliberately *not*
portable the way cones are — each is authored in its own provider's vocabulary rather
than behind a shared `allowedTools` abstraction, because flattening three permission
models into one would lie about what each backend actually enforces.
...
code in the host process (`README.md`, "Design notes": "not a sandbox"); the only thing
...
- A `return` at the end that summarizes what happened in a shape a human can skim —
  match the pattern in `examples/review.workflow.js`, which filters down to only
  confirmed findings rather than returning everything.
